...
export function TopNavBar() {
  ...
  return (
    <header className="sticky top-0 z-30 bg-surface/80 backdrop-blur-xl border-b border-outline-variant/30 h-16 w-full">
      <div className="flex items-center justify-between px-8 h-full max-w-[1680px] w-full mx-auto">
        {/* Search Bar on Left */}
        ...
        <div className="flex items-center gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="w-10 h-10 rounded-full flex items-center justify-center text-outline hover:text-on-surface hover:bg-surface-container-high transition-colors"
            title="Filter Configurations"
          >
            <span className="material-symbols-outlined">tune</span>
          </motion.button>

          {/* Icon Action: Notifications with Unread Ping */}
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="relative w-10 h-10 rounded-full flex items-center justify-center text-outline hover:text-on-surface hover:bg-surface-container-high transition-colors"
            title="Notifications"
          >
          ...
          {/* Primary Trailing Action Button */}
          <motion.button
            whileHover={{ scale: 1.02, filter: "brightness(1.05)" }}
            whileTap={{ scale: 0.98 }}
            className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-br from-[#E6399B] to-[#7C3AED] text-white text-label-md font-label-md font-semibold shadow-md shadow-primary-container/20 transition-all"
          >
          ...
          <div className="relative" ref={menuRef}>
            <motion.div
              ...
              className="flex items-center gap-3 pl-2 py-1 pr-3 rounded-full bg-surface-container hover:bg-surface-container-high border border-outline-variant/30 cursor-pointer transition-colors"
            >
              <div className="w-8 h-8 rounded-full overflow-hidden ring-1 ring-primary/40 bg-surface-container-highest relative flex items-center justify-center">
                {session?.user?.image ? (
                  <Image
